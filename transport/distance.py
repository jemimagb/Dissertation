...
import stations
from google_directions import GoogleDirections
...
def rail_distance(origin, destination, units='km'):
    ''' Uses the site railmiles as an unofficial API. Not ideal '''
    ...
    query = {'origin': origin,
             ...
             'type': 'text',
             'shortestroutes': 'on'}
    page = requests.post('https://my.railmiles.me/mileage-engine/', data=query)

    if "Error: " in page.text:
        raise Exception('railmiles.me returned an error')
